...
export class VendorController {
    constructor(
        ...
        @inject(TOKENS.ResendOtpUseCase) private resentOtp: IResendOtpUseCase,
    ...
    async register(req: CustomRequest, res: Response): Promise<void> {
        try {
            const vendorData: CreateUserDTO = req.body;
            if (!vendorData.email || !vendorData.password || !vendorData.firstName || !vendorData.lastName || !vendorData.phone) {
                throw new AppError('Name, email and password are required', HttpStatusCode.BAD_REQUEST);
            }

            const newVendor = await this.registerUser.execute(vendorData);
            ResponseHandler.success(res, 'Vendor registration in progress', newVendor, HttpStatusCode.OK);
        } catch (error: any) {
            throw error;
        }
    }
    ...
    async resendOtp(req: CustomRequest, res: Response): Promise<void> {
        try {
            ...
            if (!userId) {
                throw new AppError('Vendor userId is required', HttpStatusCode.BAD_REQUEST);
            }
            const result = await this.resentOtp.execute(userId);
    ...
    async login(req: CustomRequest, res: Response): Promise<void> {
        try {
            ...
            ResponseHandler.success(res, 'Vendor login successful', user, HttpStatusCode.OK);
        } catch (error: any) {
            throw error;
        }
    }
    ...
    async updateProfile(req: CustomRequest, res: Response): Promise<void> {
        try {
            ...
            const vendorData: UpdateUserDTO = req.body;
            const updatedVendor = await this.updateUser.execute(vendorId, vendorData);

            ResponseHandler.success(res, 'Vendor profile updated successfully', updatedVendor, HttpStatusCode.OK);
        } catch (error: any) {
            throw error;
        }
    }
    ...
    async forgotPassword(req: CustomRequest, res: Response): Promise<void> {
        try {
            ...
            if (!email) {
                throw new AppError('Email is missing in request body', HttpStatusCode.BAD_REQUEST);
            }
    ...
    async updatePassword(req: CustomRequest, res: Response): Promise<void> {
        try {
            const { email, password } = req.body;
            if (!email || !password) {
                throw new AppError('Email and new password are required.', HttpStatusCode.BAD_REQUEST);
            }

            await this.updatePass.execute(email, password);
            ResponseHandler.success(res, 'Vendor password updated successfully', null, HttpStatusCode.OK);
        } catch (error: any) {
            throw error;
        }
    }
